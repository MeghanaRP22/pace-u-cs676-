...
def sigmoid(z):
    ...
    Filled in: sigma(z) = 1 / (1 + e^-z), but computed piecewise to avoid
    overflow. For z >= 0 that formula is safe (e^-z <= 1). For z < 0 we use
    the algebraically identical e^z / (1 + e^z) instead, which only ever
    exponentiates a non-positive number. Each half is safe on its own half
    of the input; stitching them back together with a mask gives a sigmoid
    that never overflows, however extreme z gets.
...
def fit_gradient_descent(X_design, y, lr=LEARNING_RATE, n_iter=N_ITERATIONS):
    ...
    Filled in: identical shape to homework 01's loop — predict, record loss,
    compute gradient at the current beta, step all coefficients downhill at
    once. Only the prediction (sigmoid instead of linear) and the loss
    (log-loss instead of MSE) differ; the update rule itself never changes.
